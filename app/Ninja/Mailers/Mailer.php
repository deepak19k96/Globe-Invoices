...
class Mailer
{
    ...
    public function sendTo($toEmail, $fromEmail, $fromName, $subject, $view, $data = [])
    {
        ...
        if (config('services.postmark')) {
            ...
            return $this->sendgridMail($toEmail, $fromEmail, $fromName, $replyEmail, $subject, $views, $data);
        } else {
    ...
    private function sendgridMail ($toEmail, $fromEmail, $fromName, $replyEmail, $subject, $views, $data){
//        if (isset($data['body'])){
            require_once (base_path('/vendor/sendgrid-php/sendgrid-php.php'));
            $html_content = view($views[0], $data)->render();
            $sendgrid_email = new \SendGrid\Mail\Mail();
            $sendgrid_email->setFrom($fromEmail, $fromName);
            $sendgrid_email->setSubject($subject.' hello');
            $sendgrid_email->addTo($toEmail, $toEmail);
            $sendgrid_email->addContent('text/html', $html_content);
   
              // Handle invoice attachments
            if (! empty($data['pdfString']) && ! empty($data['pdfFileName'])) {   

                $attachment=$this->createSendGridAttachment($data['pdfString'],$data['pdfFileName']);
                $sendgrid_email->addAttachment($attachment);
            }
            if (! empty($data['ublString']) && ! empty($data['ublFileName'])) {
                $attachment=$this->createSendGridAttachment($data['ublString'],$data['ublFileName']);
                $sendgrid_email->addAttachment($attachment);
            }
            if (! empty($data['documents'])) {
                foreach ($data['documents'] as $document) {
              
                $attachment=$this->createSendGridAttachment($document['data'],$document['name']);
                $sendgrid_email->addAttachment($attachment);
                }
            }
