...
class InvoiceController extends BaseController
{
    ...
    public function __construct(InvoiceRepository $invoiceRepo, ClientRepository $clientRepo, InvoiceService $invoiceService, DocumentRepository $documentRepo, RecurringInvoiceService $recurringInvoiceService, PaymentService $paymentService, TemplateService $templateService)
    {
        // parent::__construct();
    ...
    public function store(CreateInvoiceRequest $request)
    {
        ...
        if (isset($data['send_sms']) && $data['send_sms'] == 'on'){
            $type='invoicesms'; //template name Todo later on we will make dynamic for multiple templates
            $account = Auth::user()->account->load('country');         
            $client =$invoice->client;
            $message =$account->getEmailTemplate($type);
         //   $subject =$account->getEmailSubject($type);         
            ...
            $message = $this->templateService->processVariables($message, $variables);

           /* $message = 'Hello ' . json_decode($data['data'])->invoice->client->contacts[0]->first_name . ' ' .
                json_decode($data['data'])->invoice->client->contacts[0]->last_name . ",\n" .
                'To view your invoice for $' . json_decode($data['data'])->invoice->amount . ', ' .
                'go to the link below.' ."\n" . env('APP_URL') . '/view/' . $invoice->invitations[0]->invitation_key . "\n" . 'Regards,' . "\n" .
                $invoice->account->getDisplayName();   */
            Utils::send_sms('+91' . $invoice->client->contacts[0]->phone, strip_tags($message));
        }
        $entityType = $invoice->getEntityType();
