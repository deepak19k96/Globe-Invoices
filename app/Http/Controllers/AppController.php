...
class AppController extends BaseController
{
    ...
    public function doSetup()
    {
        ...
        $_ENV['TWILIO_PHONE_NUMBER'] =$sms['TWILIO_PHONE_NUMBER'];
        $_ENV['TWILIO_SID'] =$sms['TWILIO_SID'];
        $_ENV['TWILIO_AUTH_TOKEN'] =$sms['TWILIO_AUTH_TOKEN'];
    ...
    public function updateSetup()
    {
        ...
        if ($mail) {
            ...
            $_ENV[$prefix . 'SENDGRID_API_KEY'] = $mail['apikey'];
            $_ENV['MAILGUN_DOMAIN'] = $mail['mailgun_domain'];
            $_ENV['MAILGUN_SECRET'] = $mail['mailgun_secret'];
        }
    ...
    private function testMail($mail)
    {
        ...
        if ($mail['driver'] = 'sendgrid'){
            require_once (base_path('/vendor/sendgrid-php/sendgrid-php.php'));
            $sendgrid_email = new \SendGrid\Mail\Mail();
            $sendgrid_email->setFrom($email, $email);
            $sendgrid_email->setSubject('Test');
            $sendgrid_email->addTo('georton@live.com', 'Test');
            $sendgrid_email->addContent('text/plain', 'This is just a test');
            $sg = new \SendGrid($mail['apikey']);
            try{
                $response = $sg->send($sendgrid_email);
                return $response->statusCode() == 202 ? 'Sent' : 'Error';
