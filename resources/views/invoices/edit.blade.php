...
@section('content')
        ...
        @if (Auth::user()->canCreateOrEdit(ENTITY_INVOICE, $invoice))
            ...
            @else
				@if (!$invoice->is_deleted)
					...
					@if (! $invoice->trashed())
						{!! Button::info(trans("texts.email_{$entityType}"))->withAttributes(array('id' => 'emailButton', 'onclick' => 'onEmailClick()'))->appendIcon(Icon::create('send')) !!}
						{{--{!! Button::info(trans("texts.sms_{$entityType}"))->withAttributes(array('id' => 'smsButton', 'onclick' => 'onSMSClick()'))->appendIcon(Icon::create('envelope')) !!}--}}
					@endif
                    @if ($invoice->id)
                        {!! DropdownButton::normal(trans('texts.more_actions'))->withContents($invoice->present()->moreActions())->dropup() !!}
	...
	function onConfirmEmailClick() {
	    console.log('EMAIL Confirm Clicked');
    ...
    function hasPhoneNumber() {
        ...
        for (var i=0; i<client.contacts().length; i++) {
            ...
            var phone_number = contact.phone();
            if (phone_number !== '') {
